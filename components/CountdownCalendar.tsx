...
const CountdownCalendar: React.FC<CountdownCalendarProps> = ({ targetDate }) => {
  ...
  /* =============================
     Calendar logic - November 2026
     Nov 1, 2026 is Sunday
     Week starts on Monday
     ============================= */
  const daysInMonth = 30;
  const startOffset = 6; // Monday(0) ... Sunday(6)
  const eventDay = 21;

  const calendarCells = Array.from(
    { length: daysInMonth + startOffset },
  ...
  return (
    <section className="bg-[#F5EBE0] py-16 px-6 relative overflow-hidden">
      ...
      <div className="text-center mb-12">
        <h3 className="font-serif text-2xl text-[#7B8C6E] mb-8 uppercase tracking-widest">
          Faltan Solo...
        </h3>
        ...
        <div className="grid grid-cols-4 gap-2 max-w-sm mx-auto">
          {[
            ...
            <div
              key={idx}
              className="bg-white/80 backdrop-blur-sm rounded-lg py-4 shadow-sm border border-[#D4AF37]/20 hover:shadow-md transition-all"
            >
              <div className="text-2xl font-bold text-[#B8860B]">
                {item.value}
              </div>
              <div className="text-[10px] uppercase tracking-tighter text-[#7B8C6E] font-semibold">
                {item.label}
              </div>
            </div>
          ))}
      ...
      <div className="bg-white p-6 rounded-2xl shadow-lg max-w-sm mx-auto border border-[#E8D5C4]">
        <h4 className="font-serif text-xl text-[#B8860B] mb-4 text-center">
          Noviembre 2026
        </h4>

        <div className="grid grid-cols-7 gap-1 text-center text-sm font-sans">
          {['L', 'M', 'M', 'J', 'V', 'S', 'D'].map((day, idx) => (
            <div key={idx} className="font-bold text-[#7B8C6E] pb-2">
              {day}
            </div>
          ))}
